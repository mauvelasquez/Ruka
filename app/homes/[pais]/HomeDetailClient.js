...
export default function HomeDetailClient({ id }) {
  ...
  const handleRequest = async e => {
    ...
    const isVerified = user?.verification_status === 'verified' || user?.verification_status === 'id_verified'
    if (!isVerified) {
      setValidationBlock('unverified')
      setErr('Debes verificarte para enviar una solicitud')
      return
    }

    // Validación: hogar inscrito
    const myHome = homes.find(h => h.userId === user.id)
    if (!myHome) {
      setValidationBlock('no_home')
      setErr('Debes registrar tu hogar antes de solicitar un intercambio')
      return
    }
    ...
    const result = await sendExchangeRequest({
      ...
      toHomeId:   home.id,
      fromHomeId: myHome.id,
      message:    form.message,
    })
    if (result) { setSent(true) }
    else setErr('No se pudo enviar la solicitud. Por favor intenta de nuevo.')
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            ...
            <div className="flex items-center gap-2 mb-2 flex-wrap">
              ...
              {home.is_demo && (
                <span className="bg-purple-100 text-purple-700 text-xs font-bold px-2.5 py-1 rounded-full border border-purple-200">
                  Perfil de ejemplo · Así podría verse tu casa
                </span>
              )}
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {/* TODO: re-habilitar valoraciones — sección de reseñas con estrellas comentada temporalmente */}
          </div>
                  ...
                  <form onSubmit={handleRequest} className="space-y-4">
                    ...
                    {err && (
                      <div className="space-y-2">
                        ...
                        {validationBlock === 'unverified' && (
                          <Link href="/onboarding" className="inline-flex items-center gap-1 text-xs font-bold text-forest hover:underline">
                            <Shield className="w-3 h-3" /> Verificar mi identidad →
                          </Link>
                        )}
                        {validationBlock === 'no_home' && (
                          <Link href="/dashboard/property/new" className="inline-flex items-center gap-1 text-xs font-bold text-forest hover:underline">
                            <HomeIcon className="w-3 h-3" /> Publicar mi hogar →
                          </Link>
                        )}
                      </div>
                    )}
