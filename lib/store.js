...
export function AppProvider({ children }) {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      if (event === 'SIGNED_IN' && session?.user) {
        ...
        if (profile) {
          ...
          // Usuario Google con perfil confirmado que llegó a /onboarding → ir al dashboard
          if (profile.status === 'confirmed' && typeof window !== 'undefined' && window.location.pathname === '/onboarding') {
            window.location.href = '/dashboard'
          }
        } else {
          // Perfil no existe aún — crear perfil pending para disparar onboarding
  ...
  const completeOnboarding = async ({ profileData, homeData }) => {
    try {
      ...
      const avatar = profileData.avatar || getAvatarChileno(session.user.user_metadata?.name || '')
