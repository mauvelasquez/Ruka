...
export function AppProvider({ children }) {
      ...
      if ((event === 'SIGNED_IN' || event === 'TOKEN_REFRESHED') && session?.user) {
        ...
        if (profile) {
          ...
          if (session.user.app_metadata?.provider === 'google') {
            ...
            sendMetaEvent({ event_name: 'CompleteRegistration', email: session.user.email, phone: session.user.user_metadata?.phone || null })
  ...
  const register = async ({ name, email, password, phone, country_user }) => {
    try {
      ...
      sendMetaEvent({ event_name: 'CompleteRegistration', email, phone: phone || null })
