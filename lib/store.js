  ...
  useEffect(() => {
    ...
    const fetchWithTimeout = (table, query, ms = 4000) => {
      ...
      const timer = setTimeout(() => {
        console.error(`[store] ${table} timed out after ${ms}ms — aborting (likely RLS or network hang)`)
        ctrl.abort()
      }, ms)
      console.log(`[store] fetching ${table}…`)
      return query.abortSignal(ctrl.signal)
        .then(res => {
          clearTimeout(timer)
          if (res.error) {
            console.error(`[store] ${table} returned error:`, res.error.message, res.error.code)
          } else {
            console.log(`[store] ${table} OK — ${res.data?.length ?? 0} rows`)
          }
          return res
        })
        .catch(err => {
    ...
    const loadPublicData = async () => {
      ...
      try {
        // Queries exactas — todas públicas (anon key, sin filtros de usuario):
        //   homes:         SELECT * FROM homes
        //   profiles:      SELECT * FROM profiles
        //   travel_wishes: SELECT * FROM travel_wishes
        // Si RLS bloquea una de estas sin retornar error (solo cuelga), el timeout la corta.
        const [homesResult, usersResult, wishesResult] = await Promise.allSettled([
          fetchWithTimeout('homes',         supabase.from('homes').select('*')),
        ...
        if (homesResult.status === 'rejected')
          console.error('[store] homes fetch failed:', homesResult.reason)
        ...
        if (homesResult.status === 'fulfilled' && homesResult.value.data)
          setHomes(homesResult.value.data.map(normalizeHome))
    ...
    // ── Auth init: getSession() en lugar de depender de INITIAL_SESSION ──────────
    // getSession() lee cookies directamente (<5ms, sin network, sin contención de lock
    // prolongada). Resolvemos auth primero y LUEGO iniciamos loadPublicData para
    // evitar que los 3 queries compitan con el lock interno de gotrue-js.
    const initAuth = async () => {
      ...
      // Iniciar carga de datos DESPUÉS de que el lock de auth está libre
      loadPublicData()
    }

    // Fallback de seguridad: si getSession() no resuelve en 5s (edge case extremo)
