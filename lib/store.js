...
export function AppProvider({ children }) {
  // createClient() is memoized — safe to call on every render, returns same instance
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      if (event === 'INITIAL_SESSION') {
        ...
        if (session?.user) {
          const gen = ++authGen
          // Batch setUser + setAuthReady in the same sync block so React renders
          // them together — avoids a frame where ready=true but user=null (which
          // would trigger an erroneous redirect to /login).
          setUser(prev => prev ?? {
            ...session.user,
        ...
        if (profile) {
          setUser({ ...session.user, ...normalizeUser(profile) })
          // Sync Google email (belt-and-suspenders — auth/callback handles this server-side too)
