...
export function AppProvider({ children }) {
  ...
  useEffect(() => {
    ...
    const loadPublicData = async () => {
      ...
      // homes tiene una política RLS lenta (~7-9s). Lo cargamos en background
      // para que no bloquee dataReady — el dashboard abre rápido y los hogares
      // aparecen cuando llegan.
      fetchWithTimeout('homes', supabase.from('homes').select('*'), 12000)
        .then(res => { if (res?.data) setHomes(res.data.map(normalizeHome)) })
        .catch(err => console.error('[store] homes background failed:', err))
