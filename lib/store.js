...
export function AppProvider({ children }) {
  ...
  useEffect(() => {
    ...
    const loadPublicData = async () => {
      ...
      try {
        ...
        const [homesResult, usersResult, wishesResult] = await Promise.allSettled([
          fetchWithTimeout('homes',         supabase.from('homes').select('*'), 10000),
    ...
    const initAuth = async () => {
      console.log('[store] initAuth start — calling getSession()')
      try {
        const { data: { session } } = await supabase.auth.getSession()
        console.log('[store] getSession() resolved — session:', session ? 'yes' : 'null')
