// Server-side email helpers using Resend directly.
...
// Lazy getter — avoids module-level throw when RESEND_API_KEY is absent during build
function getResend() {
...
export async function sendHomeSavedEmail(userEmail, homeName, homeId) {
  ...
  if (result.error) {
    console.error('[emails] sendHomeSavedEmail failed:', result.error)
  } else {
    console.log('[emails] sendHomeSavedEmail sent, id:', result.data?.id)
  }
  return result
}
...
export async function sendVerificationSuccessEmail(userEmail, userName) {
  ...
  if (result.error) {
    console.error('[emails] sendVerificationSuccessEmail failed:', result.error)
  } else {
    console.log('[emails] sendVerificationSuccessEmail sent, id:', result.data?.id)
  }
  return result
}
